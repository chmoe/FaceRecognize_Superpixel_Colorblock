...
class SLICProcessor(object):
    ...
    def save_lab_image(self, path, lab_arr):
        ...
        rgb_arr = color.lab2rgb(lab_arr)  # 出来的是小数
        print(rgb_arr)
        rgb_arr_cv = rgb_arr[:, :, [2, 1, 0]]
        rgb_arr = rgb_arr * 255

        for cluster in self.clusters:
            print("cluster", cluster)
            print("cluster.h", cluster.h)
            print("cluster.w", cluster.w)
            print("rgb[h][w]", rgb_arr[cluster.h][cluster.w])
            print("rgb.shape", rgb_arr.shape)
            print("rgb[0][0]", rgb_arr[22][116])
            # cv2.putText(rgb_arr, self.rgb2hex(rgb_arr[cluster.h][cluster.w]), (cluster.h, cluster.w),
            #             None, 0.5, (0, 255, 0), 1, cv2.LINE_AA)
            cv2.putText(rgb_arr, self.rgb2hex(rgb_arr[cluster.h][cluster.w]), (cluster.h, cluster.w),
        ...
        #             cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1, cv2.LINE_AA)
        # print(type(rgb_arr))
        # io.imsave(path, rgb_arr[:, :, [2, 1, 0]])
        print(rgb_arr)
        cv2.imwrite(path, rgb_arr[:, :, [2, 1, 0]])
...
if __name__ == '__main__':
    for i in range(37):
        ...
        p = SLICProcessor('./result/{}.png'.format(i), 40, 140)
        p.iterate_10times(i)
